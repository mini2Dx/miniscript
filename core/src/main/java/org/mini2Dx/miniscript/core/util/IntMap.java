...
public class IntMap<V> implements Iterable<IntMap.Entry<V>> {
	...
	public IntMap (int initialCapacity, float loadFactor) {
		...
		int tableSize = IntSet.tableSize(initialCapacity, loadFactor);
	...
	public void shrink (int maximumCapacity) {
		...
		int tableSize = IntSet.tableSize(maximumCapacity, loadFactor);
	...
	public void ensureCapacity (int additionalCapacity) {
		int tableSize = IntSet.tableSize(size + additionalCapacity, loadFactor);
		if (keyTable.length < tableSize) resize(tableSize);
	}
	...
	static private class MapIterator<V> {
		...
		public void reset () {
			...
			if(this.keyTable == null || this.keyTable.length < map.keyTable.length) {
				this.keyTable = new int[map.keyTable.length];
			}
			if(this.valueTable == null || this.valueTable.length < map.valueTable.length) {
				this.valueTable = (V[])new Object[map.valueTable.length];
			}
	...
	static public class Entries<V> extends MapIterator<V> implements Iterable<Entry<V>>, Iterator<Entry<V>> {
		...
		public Entry<V> next () {
			...
			if (nextIndex == INDEX_ZERO) {
				entry.key = 0;
				entry.value = this.zeroValue;
			} else {
				entry.key = keyTable[nextIndex];
				entry.value = this.valueTable[nextIndex];
			}
	...
	static public class Values<V> extends MapIterator<V> implements Iterable<V>, Iterator<V> {
		...
		public V next () {
			...
			if (nextIndex == INDEX_ZERO)
				...
				value = this.valueTable[nextIndex];
			currentIndex = nextIndex;
			findNextIndex();
			return value;
		}
	...
	static public class Keys extends MapIterator {
		...
		public int next () {
			if (!hasNext) throw new NoSuchElementException();
			if (!valid) throw new RuntimeException("#iterator() cannot be used nested.");
			int key = nextIndex == INDEX_ZERO ? 0 : this.keyTable[nextIndex];
			currentIndex = nextIndex;
			findNextIndex();
			return key;
		}
